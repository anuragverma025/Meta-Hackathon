...
def compute_reward(
    ...
    failed_attempts: Optional[Dict[str, int]] = None,
    ...
    Parameters
    ----------
    failed_attempts : dict mapping ticket_id -> prior fail count, used by
                      reward_recovery to bonus successful retries.
    ...
    # -- Subtask decomposition completion (Part 2) -------------------------
    subtask_total = reward_subtask_completion(tool_call_logs)

    # -- Recovery bonus for previously-failed tickets (Part 2) ------------
    recovery_total = reward_recovery(tool_call_logs, failed_attempts)

    # -- Specialized per-agent bonuses ------------------------------------
    tactical_total = reward_tactical_agent(step_result, tool_call_logs)
    strategic_total = reward_strategic_agent(step_result, tool_call_logs)
    ...
    # Add non-weighted signals (unscaled additive terms)
    total = (
        weighted
        + hallucination_total
        + exploration_total * 0.1
        + subtask_total
        + recovery_total
        + tactical_total
        + strategic_total
